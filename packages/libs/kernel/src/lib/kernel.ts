...
export class Kernel {
  ...
  /**
   * Submit a function that should dispatch a mutation to the pullable source
   * @param id Source Id
   * @param action Action name
   * @param payload Any payload
   */
  mutate(id: string, action: string, payload: any) {
    // Get the global subject reference
    const ref = this._contexts[id];
